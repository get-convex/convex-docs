...
type. If you're using [TypeScript](/docs/typescript.mdx), adding a schema will
...
and more. [Argument validation](/docs/functions/args-validation.mdx) and schemas
...
[`Infer` type helper](/functions/args-validation#extracting-typescript-types).
...
[`db.insert(table, document)`](/api/interfaces/server.DatabaseWriter#insert) now
understand your schema. Additionally
[database queries](/docs/database/reading-data.mdx) will now return the correct
