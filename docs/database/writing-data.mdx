...
[`db.insert`](/api/interfaces/server.DatabaseWriter#insert) method:
...
1. The [`db.patch`](/api/interfaces/server.DatabaseWriter#patch) method will
...
2. The [`db.replace`](/api/interfaces/server.DatabaseWriter#replace) method will
   replace the existing document entirely, potentially removing existing fields:
...
the [`db.delete`](/api/interfaces/server.DatabaseWriter#delete) method.
