...
- [Streaming Export](/docs/database/import-export/streaming.md) (Fivetran or
  Airbyte) does not understand snapshot imports, similar to table deletion and
  creation and some schema changes. We recommend resetting streaming export sync
...
- Avoid changing the ZIP file between downloading it from Snapshot Export and
  ...
  with the Convex team in [Discord](http://convex.dev/community).
- Snapshot import is not always supported when importing into a deployment that
  ...
  - The import may work, especially when importing a ZIP snapshot from a
    ...
    backup.
  - Reach out in [Discord](http://convex.dev/community) if you encounter issues,
    as there may be a workaround.
