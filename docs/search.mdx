...
Unlike normal
[document queries](/docs/database/reading-data.mdx#querying-documents), search
queries look _within_ a string field to find the keywords. This can be used to
...
Like [database indexes](/docs/database/indexes/indexes.md), search indexes are a
...
This is just a normal [database read](/docs/database/reading-data.mdx) that
...
[filter results](/docs/database/reading-data.mdx#filtering) using the
...
[retrieve the results](/docs/database/reading-data.mdx#retrieving-results) using
[`.collect()`](/api/interfaces/server.Query#collect),
...
Convex full-text search is designed to power as-you-type search experiences. To
achieve this, full-text search automatically applies fuzzy and prefix matching
rules to search terms! This means that documents matched by a search query do
not necessarily need to exactly match any of the query's terms.

Depending on the length of search terms, a fixed number of typos is permitted
between matches. Typos are defined in terms of
[Levenshtein distance](https://en.wikipedia.org/wiki/Levenshtein_distance). The
specific typo-tolerance rules are:

- Terms with length `<=` 4 allow no typos
- Terms with 5 `<` length `<=` 8 allow 1 typo
- Terms with length `>` 8 allow 2 typos

For example, the expression `search("body", "hello something")` would match the
following documents:
...
In addition to typo-tolerance, the final search term has _prefix search_
enabled, matching any term that is a prefix of the original term. For example,
the expression `search("body", "r")` would match the documents:
...
**Relevance order is subject change.** The relevance of search results and the
exact typo-tolerance rules Convex applies is subject to change to improve the
...
[limit of 32 indexes per table](/docs/database/indexes/indexes.md#limits).
