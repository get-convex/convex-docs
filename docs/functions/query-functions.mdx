...
See [argument validation](/docs/functions/args-validation.mdx) for the full list
...
[Convex type](/docs/functions/args-validation.mdx) which will be automatically
...
- To fetch from the database use the `db` field. Note that we make the handler
  ...
  Read more about [Reading Data](/docs/database/reading-data.mdx).
...
use [actions](/docs/functions/actions.mdx).
