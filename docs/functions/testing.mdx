...
- Runtime built-ins. Most of your functions are written for the
  [Convex default runtime](/docs/functions/runtimes.mdx), while Vitest runs your
  tests in your local Node.js runtime. You should always test new code manually
  to make sure it doesn't use built-ins not available in the Convex runtime.
- Some features have only simplified semantics, namely:
  ...
  - [Vector search](/docs/vector-search.mdx) returns results sorted by cosine
