...
Like [database indexes](/docs/database/indexes/indexes.md), vector indexes are a
data structure that is built in advance to enable efficient querying. Vector
...
1. `vectorField` string
   ...
   - The field must be of type `v.array(v.float64())` (or a union in which one
     of the possible types is `v.array(v.float64())`)
1. `dimensions` number
   - The fixed size of the vectors index. If you're using embeddings, this
     ...
     OpenAI). `dimensions` must be between `2` and `2048`.
...
1. Use [`ctx.vectorSearch`](api/interfaces/server.GenericActionCtx#vectorsearch)
   to fetch the IDs of similar documents
...
[limit of 32 indexes per table](/docs/database/indexes/indexes.md#limits). In
addition you can have up to 4 vector indexes per table.
