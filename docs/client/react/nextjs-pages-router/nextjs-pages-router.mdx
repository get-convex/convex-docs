...
[ConvexHttpClient](/api/classes/browser.ConvexHttpClient) to call your query and
mutation functions:
...
[fully reactive](/docs/functions/query-functions.mdx#caching-reactivity). Convex
...
[ConvexHttpClient](/api/classes/browser.ConvexHttpClient) to call query
functions just like you would in [API routes](#api-routes).

To make authenticated requests to Convex during server-side rendering, the
[ConvexHttpClient](/api/classes/browser.ConvexHttpClient) instance needs
...
[`ConvexHttpClient.setAuth`](/api/classes/browser.ConvexHttpClient#setauth) to
