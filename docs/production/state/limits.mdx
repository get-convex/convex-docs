...
| ----------------- | ------------- | -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Storage           | 0.5 GiB       | 50 GiB included<br/>$0.20/month per additional GiB | Includes database rows and indexes but not files or snapshots. Per-document size is rounded up to the nearest 1 KiB.                            |
| Bandwidth         | 1 GiB/month   | 50 GiB/month included<br/>$0.20 per additional GiB | Document and index data transferred between Convex functions and the underlying database. Per-document size is rounded up to the nearest 1 KiB. |
...
| [Log streaming](/docs/production/integrations/log-streams.mdx) limits | 4096 logs, flushed every 10 seconds | 4096 logs, flushed every 10 seconds                       | How many logs can be buffered when streaming                                                                  |
...
|              |        | Notes                                    |
| ------------ | ------ | ---------------------------------------- |
| Data read    | 16 MiB | Each document read rounded up to 1 KiB.  |
| Data written | 8 MiB  | Each document write rounded up to 1 KiB. |
...
|           | Starter     | Professional                                        |
| --------- | ----------- | --------------------------------------------------- |
| Storage   | 1 GiB       | 100 GiB included<br/>$0.03/month per additional GiB |
| Bandwidth | 1 GiB/month | 50 GiB/month included<br/>$0.30 per additional GiB  |
