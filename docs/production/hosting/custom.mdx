...
description:
  "Host your frontend on any static hosting provider, such as GitHub."
...
In this guide we'll setup a local build, but your hosting provider might support
a remote build. For example see
...
1. Configure <JSDialectFileName name="vite.config.ts" />:
...
2. It invokes the the frontend framework build process, via `npm run build`. The
