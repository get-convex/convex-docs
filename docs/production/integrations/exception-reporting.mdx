...
- `func_runtime`: One of the [function runtimes](/functions/runtimes) -
  `["default", "node"]`
- `request_id`: The
  [request id](/functions/debugging#finding-relevant-logs-by-request-id) of the
  function that errored.
- `server_name`: The name of the deployment. e.g. `happy-animal-123`
- `environment`: One of `["prod", "dev", "preview"]`
- `user`: If the function is [authenticated](/auth), then the
