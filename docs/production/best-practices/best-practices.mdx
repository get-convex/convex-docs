...
to your convex functions. Check this in to your git repo. This way your code
will typecheck without needing to run `npx convex codegen` or `npx convex dev`
(which includes codegen) first.

If someone needs to make changes to the frontend of an an app or website that
uses they can run it against the production deployment by setting an environment
variable, without ever needing to run the Convex CLI tool.
