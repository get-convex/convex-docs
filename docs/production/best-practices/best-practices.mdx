...
We recommend the
[no-floating-promises](https://eslint.org/docs/latest/rules/no-floating-promises)
...
Search for `ctx.runQuery`, `ctx.runMutation`, and
`ctx.runAction in your Convex codebase. Also search for `ctx.scheduler`and check the`crons.ts` file. Ensure all of these use`internal.foo.bar`functions instead of`api.foo.bar`
functions.
